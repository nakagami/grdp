...
func (x *X224) Write(b []byte) (n int, err error) {
	...
	slog.Debug(fmt.Sprintf("x224 write:", hex.EncodeToString(buff.Bytes())))
...
func (x *X224) recvConnectionConfirm(s []byte) {
	slog.Debug("x224 recvConnectionConfirm ", hex.EncodeToString(s))
	r := bytes.NewReader(s)
...
func (x *X224) recvData(s []byte) {
	slog.Debug(fmt.Sprintf("x224 recvData", hex.EncodeToString(s), "emit data"))
