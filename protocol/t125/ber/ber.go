...
func ReadApplicationTag(tag uint8, r io.Reader) (int, error) {
	...
	if tag > 30 {
		if bb != (CLASS_APPL|PC_CONSTRUCT)|TAG_MASK {
			return 0, errors.New(fmt.Sprintf("ReadApplicationTag error tag=0x%x,bb=0x%x", tag, bb))
		}
		bb, _ := core.ReadUInt8(r)
		if bb != tag {
		...
		if bb != (CLASS_APPL|PC_CONSTRUCT)|(TAG_MASK&tag) {
			return 0, errors.New(fmt.Sprintf("ReadApplicationTag error valiable length tag=0x%x,bb=0x%x", tag, bb))
		}
	}
	return ReadLength(r)
}
